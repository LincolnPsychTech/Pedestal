function buildAdaptationExperiment(Var)
%% THIS IS OLD CODE
%get extra info from Var
noVariables = length(Var);
totalBlocks = inputdlg('How many blocks would you like?');
totalBlocks = str2num(totalBlocks{:});
% for variable = 1:noVariables
%     varLevels(variable) = length(Var.Conditions(variable));
% end
% totalBlocks = sum(varLevels); %total number of conditions = number of blocks


for block = 1:totalBlocks %currently this just goes through every block, but in the final thing this will be triggered by clicking on the block in the block menu
    %Get timings per block
    msgbox(['Define stimuli for block ' num2str(block)])
    durQuests = {'Adapt stimulus duration...','Top-up stimulus duration...','Test stimulus duration...', 'ISI duration...', 'Number of trials...'};
    defaultAns = {'30000' '7000' '5000' '500' '10'};
    stimValues{:,block} = inputdlg(durQuests, ['Specify stimulus duration for block ' num2str(block) ' (milliseconds)'], 1, defaultAns);
    %Link blocks to conditions
    for variable = 1:noVariables
        condTemp = conditions{variable};
        blockClass{block,variable} = listdlg('PromptString',['Which condition of ' varNames(variable) ' is block ' num2str(block) ' in?'], 'ListString', condTemp,'SelectionMode','single');
        %answers here can be indexed back into conditions
    end
    %% adapt
...
if saveQuest == 'Yes'
    ...
    save([saveLocation,'\',saveName{:}], 'blockClass', 'adaptDir', 'testDir', 'stimValues', 'stimType', 'otherStimInfo');
end

runQuest = questdlg('Would you like to run your experiment?');
if runQuest == 'Yes'
